...
2. **Elimination of Duplicate and Empty Client-Side Notifications & Subscription Cache Sync**
   * *Resolution*: Simplified `createNotification` in `backend/services/notificationService.js` and removed manual, partial `emitToUser(..., 'notification:new')` calls inside `voteController.js`, `answerController.js`, and `adminController.js`. Furthermore, updated `checkPushSubscription` inside `frontend/context/NotificationContext.js` to always upload and synchronize the active browser Web Push (VAPID) subscription to the backend database when a user is logged in (rather than only doing so if the browser's local subscription cache is empty). This prevents users from losing notifications when logging in on different accounts, after database wipes, or across restarts.
3. **Foreground Real-Time In-App Toast & Native OS Push Banners**
...
1. **Spurti Points (Sp) Incentive System**
   ...
   * *Resolution*: Established an immutable transaction logging system via `SpurtiPointLog.js` schema and exposed endpoints (`GET /api/users/me/spurti-logs`, `GET /api/admin/spurti-logs`). Integrated auto-rewards in `answerController.js` (+1 Sp on accepted, -1 Sp on unaccepted). Re-weighted real-time leaderboard ranking to combine resolution count, solved upvotes, reputation, and Sp. Developed a premium user-facing Spurti Bank dashboard on the profile page (`/users/[username]`), which displays cohort comparisons, user rank standings, Q&A health, badges list, a graphical points trend chart, and a fully paginated ledger table of transaction logs. Integrated a dynamic server-side recalculation step to automatically sync all users' Spurti Points from the database in real-time when the leaderboard is updated. Cleaned the Community layout by completely removing all badge displays from the leaderboard and moderators sections.
...
#### Older Fixes (June 8, 2026 — SP System Integrity & Data Sync)
